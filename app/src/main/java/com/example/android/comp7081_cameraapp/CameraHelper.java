...
public class CameraHelper {
    ...
    public static File getPhotoDirectory()  {
        ...
        if (externalStorageState.equals(Environment.MEDIA_MOUNTED)) {
            ...
            outputDir = new File(pictureDir, "COMP7081-CameraApp");
            if(!outputDir.exists())  {
                if(!outputDir.mkdirs()) {
    ...
    public static File generatePhotoFileWithTimeStampAndLocale(String locale, String customTag) {
        File photoFile = null;
        File outputDir = getPhotoDirectory();

        if (outputDir != null) {
            String timeStamp = new SimpleDateFormat("HHmmss_yyyyMMdd").format(new Date());
            String photoFileName = locale + "-" + customTag + "_" + timeStamp + ".jpg";
